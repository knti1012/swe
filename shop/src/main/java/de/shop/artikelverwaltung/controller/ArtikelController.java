...
public class ArtikelController implements Serializable {
	...
	private Artikel neuerArtikel; // = new Artikel(); // "Leeres Objekt" fuer
									// Eingabe
	...
	private String updateErrorMsg(RuntimeException e,
			Class<? extends Artikel> artikelClass) {
		...
		if (exceptionClass.equals(OptimisticLockException.class)) {
			if (artikelClass.equals(Artikel.class)) {
				messages.error(ARTIKELVERWALTUNG,
						MSG_KEY_UPDATE_ARTIKEL_CONCURRENT_UPDATE, null);
			}
		} 
		else if (exceptionClass.equals(ConcurrentDeletedException.class)) {
			if (artikelClass.equals(Kunde.class)) {
				messages.error(ARTIKELVERWALTUNG,
						MSG_KEY_UPDATE_ARTIKEL_CONCURRENT_DELETE, null);
			}
		}
		return null;
	}
