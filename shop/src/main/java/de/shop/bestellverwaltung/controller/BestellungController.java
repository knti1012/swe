...
public class BestellungController implements Serializable {
	...
	public String bestellen() {
		...
		if (warenkorb == null || warenkorb.getPositionen() == null
				|| warenkorb.getPositionen().isEmpty()) {
			// Darf nicht passieren, wenn der Button zum Bestellen verfuegbar
			// ist
			return JSF_DEFAULT_ERROR;
		}

		// Den eingeloggten Kunden mit seinen Bestellungen ermitteln, und dann
		// die neue Bestellung zu ergaenzen
		...
		final List<Bestellposition> neuePositionen = new ArrayList<>(
				positionen.size());
		...
		// dessen Bestellungen muessen geladen sein, weil es eine bidirektionale
		// Beziehung ist
		try {
			bestellung = bs.createBestellung(bestellung, kunde, locale);
		} catch (Exception e) {
			// Validierungsfehler KOENNEN NICHT AUFTRETEN, da Attribute durch
			// JSF validiert wurden
			// und in der Klasse Bestellung keine Validierungs-Methoden
			// vorhanden sind
			// throw new IllegalStateException(e);
			System.out
					.println("Exception wurde gefangen: erstellen einer bestellung nicht möglich");
		}
	...
	public String createBestellung() {
		try {
			...
			neueBestellung = (Bestellung) bs.createBestellung(bestellung,
					kunde, locale);
		} catch (Exception e) {
